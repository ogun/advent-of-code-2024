...
def is_safe(report):
    ...
    positive = True if report[1] > report[0] else False
    for idx, level in enumerate(report[1:]):
        previous = report[idx]
        current = level
        if positive:
            safe = True if 4 > current - previous > 0 else False
        else:
            safe = True if 0 > current - previous > -4 else False
...
def second():
    ...
    for report in reports:
        ...
        if not safe:
            sub_reports = list(itertools.combinations(report, len(report) - 1))
            for sub_report in sub_reports:
                safe |= is_safe(sub_report)
