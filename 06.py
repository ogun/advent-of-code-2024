...
for formula in formulas:
    data = list(x for x in data_all if x[0] in formula and x[1] in formula)

    all = set()
    for d in data:
        all.add(d[0])
        all.add(d[1])
    all_dict = {k: [] for k in all}
    all_dict_reverse = {k: [] for k in all}
    for d in data:
        all_dict[d[0]].append(d)
        all_dict_reverse[d[1]].append([d[1], d[0]])

    starting_point = [k for k, v in all_dict.items() if not v][0]
    end_point = [k for k, v in all_dict_reverse.items() if not v][0]
    chains = all_dict_reverse[starting_point]
    ...
    while added:
        added = False
        for chain in chains:
            branches = all_dict_reverse[chain[-1]]
            if not branches:
            ...
            for branch in branches:
                ...
                # if branch[1] in new_chain:
                #     continue

                new_chain.append(branch[1])

                if new_chain[-1] == end_point:
    ...
    order = list(reversed([x for x in finished_chains if len(x) == len(all)][0]))
    if order == formula:
        accepteds.append(formula)
    else:
        rejecteds.append(order)
